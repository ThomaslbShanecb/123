...
javascript:(function(){
  ...
  function getInitialPanelZIndex(){
    let maxZ = 1000;
    ...
    for(const el of els){
      try{
        ...
        if((pos==='fixed' || pos==='absolute' || pos==='sticky' || pos==='relative') && !Number.isNaN(z)){
          if(z > maxZ) maxZ = z;
        }
      }catch(e){}
    }
    return Math.max(1000, maxZ + 1);
  }

  const PANEL_Z_INDEX = getInitialPanelZIndex();
  ...
  wrap.style.cssText='position:fixed;inset:0;z-index:'+PANEL_Z_INDEX+';pointer-events:none;';

  const panel=document.createElement('div');
  panel.id=PANEL_ID;
  panel.style.cssText='position:absolute;top:80px;right:40px;width:420px;height:520px;min-width:280px;min-height:180px;overflow:hidden;resize:both;pointer-events:auto;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,PingFang SC,Microsoft YaHei,sans-serif;z-index:'+PANEL_Z_INDEX+';';

  const header=document.createElement('div');
  header.style.cssText='height:48px;display:flex;align-items:center;justify-content:space-between;padding:0 12px 0 14px;cursor:move;user-select:none;pointer-events:auto;';
  ...
  btns.style.cssText='display:flex;align-items:center;gap:8px;pointer-events:auto;';
  ...
  function makeBtn(txt,title){
    ...
    b.textContent=txt;
    b.title=title||txt;
    b.style.cssText='border:none;padding:6px 10px;border-radius:10px;font-size:12px;font-weight:600;cursor:pointer;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);pointer-events:auto;';
    return b;
  }
  ...
  let notesBaselineList=null;
  let currentActiveTagIndex=null;

  function buildNotesMap(currentArr){
    if(!notesBaselineList || !notesBaselineList.length || !currentArr || !currentArr.length){
      return new Map();
    }

    const baseTexts=notesBaselineList.map(x=>x.text);
    const currTexts=currentArr.map(x=>x.text);
    const m=baseTexts.length;
    const n=currTexts.length;

    const dp=Array.from({length:m+1},()=>Array(n+1).fill(0));

    for(let i=m-1;i>=0;i--){
      for(let j=n-1;j>=0;j--){
        if(baseTexts[i]===currTexts[j]){
          dp[i][j]=dp[i+1][j+1]+1;
        }else{
    ...
    const anchors=[...matches,{b:m,c:n}];
    let prevB=-1;
    let prevC=-1;

    for(const anchor of anchors){
      const baseStart=prevB+1;
      const baseEnd=anchor.b-1;
      const currStart=prevC+1;
      const currEnd=anchor.c-1;

      const baseGapLen=Math.max(0,baseEnd-baseStart+1);
      const currGapLen=Math.max(0,currEnd-currStart+1);
      const pairLen=Math.min(baseGapLen,currGapLen);

      for(let k=0;k<pairLen;k++){
        const bIdx=baseStart+k;
        const cIdx=currStart+k;
        if(baseTexts[bIdx]!==currTexts[cIdx]){
          noteMap.set(cIdx,'Edited');
        }
      }

      for(let k=pairLen;k<currGapLen;k++){
        noteMap.set(currStart+k,'New label');
      }
  ...
  function ensureBaseline(arr){
    if(notesBaselineList) return;
    const valid=(arr||[]).filter(x=>x && x.index>=0 && x.text && !x.text.startsWith('未找到'));
    if(!valid.length) return;
    notesBaselineList=valid.map(x=>({text:x.text,index:x.index}));
    notesBaseline=new Map(valid.map(x=>[String(x.index),x.text]));
  }
  ...
  function render(){
    ...
    const validArr=arr.filter(x=>x && x.index>=0 && x.text && !x.text.startsWith('未找到'));
    const noteMap=notesMode ? buildNotesMap(validArr) : new Map();
    let validCursor=0;
    ...
    arr.forEach((obj,i)=>{
      ...
      let noteText='';
      const isValid=obj && obj.index>=0 && obj.text && !obj.text.startsWith('未找到');
      if(notesMode && isValid){
        noteText=noteMap.get(validCursor)||'';
        validCursor++;
      }
  ...
  document.addEventListener('mousemove',function(e){
    ...
    const maxY=Math.max(0,window.innerHeight-(minimized?48:panel.offsetHeight));
  ...
  window.addEventListener('resize',function(){
    ...
    const maxX=Math.max(0,window.innerWidth-panel.offsetWidth);
    const maxY=Math.max(0,window.innerHeight-(minimized?48:panel.offsetHeight));
    panel.style.left=Math.max(0,Math.min(rect.left,maxX))+'px';
    panel.style.top=Math.max(0,Math.min(rect.top,maxY))+'px';
    panel.style.right='auto';
  });
